...
//	    this._backgroundSprite.bitmap = SceneManager.backgroundBitmap();
            this._backgroundSprite.bitmap =
             ImageManager.loadSystem('menu_book');
	...
	Window_MonsterInfo.prototype.drawParametersTopPanel = function(rect)
	{
		...
		for (var i = 0; i < enemy.params.length; i++)
		{
			...
			//update param based on difficulty settings
	var adjusted_data = enemy.params[i];
	var ratio = $gameVariables.value(61).clamp(0,2);
		switch (i) {
		case 0:  //mhp
			adjusted_data = Math.floor(adjusted_data*(1+ratio*ratio*0.35));
			break;
		case 1:  //mmp
			adjusted_data = Math.floor(adjusted_data*(1+ratio*0.5));
			break;
		case 2:  //atk
			adjusted_data = Math.floor(adjusted_data*(1+ratio*0.3));
			break;
	    case 3:  //def
			adjusted_data = Math.floor(adjusted_data*(1+ratio*0.15));
			break;
		case 4:  //mat
			adjusted_data = Math.floor(adjusted_data*(1+ratio*0.3));
			break;
		case 5:  //mdf
			...
			break;
		case 6:  //agi
			adjusted_data = Math.floor(adjusted_data*(1+ratio*ratio*0.1));	
			break;
		//default:
		//	adjusted_data = enemy.params[i];
		}  
			...
			this.resetTextColor();
			this.drawText(adjusted_data, x, h, w, 'right');
			h += this.lineHeight();
		}
	};
...
//doranikofu change display to show resistance
				r -= 100;
				r *= -1;
				this.drawText(r + "%", x, h, w, 'right');
			} else {
				this.drawText("0%", x, h, w, 'right');}
			h += this.lineHeight();
		}
	};
...
Window_MonsterList.prototype.lineHeight = function() {
    return 54;
};
