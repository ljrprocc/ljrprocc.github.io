...
Imported['VE - State Graphics'] = '1.01';
...
/*:
 *------------------------------------------------------------------------------ 
 * @plugindesc v1.01 - Change actors and enemies graphics when under states.
 * @author Victor Sant
 *
...
(function() {
	...
	if (Imported['VE - Basic Module']) {
		...
		VictorEngine.Parameters.ChangeCharset = eval(parameters["Change Charset"]);
		VictorEngine.Parameters.ChangeFaceset = eval(parameters["Change Faceset"]);
	};
	...
	VictorEngine.StateGraphics.loadNotes = function(data) {
		...
		this.processGraphicNotes(data);
		this.processWeaponsNotes(data);
	};
	
	VictorEngine.StateGraphics.processGraphicNotes = function(data) {
		var part1 = 'state graphic (replace|extension):[ ]*';
		var part2 = "('[^\']+'|\"[^\"]+\")[ ]*(?:,[ ]*(\\d+))?";
		var regex = new RegExp('<' + part1 + part2 + '[ ]*>', 'gi');
		var match;
		while ((match = regex.exec(data.note)) !== null) {
			this.processValues(match, data.stateGraphic);
		};
	};
	
	VictorEngine.StateGraphics.processWeaponsNotes = function(data) {
		...
		while ((match = regex1.exec(data.note)) !== null) { 
			data.stateGraphic.hideWeapon = true;
		};
		while ((match = regex2.exec(data.note)) !== null) {
			data.stateGraphic.animation = match[1];
		};
	};
	
	VictorEngine.StateGraphics.processValues = function(match, data) {
		var value;
		var regex1 = new RegExp("^('[^\']+'|\"[^\"]+\")", 'gi');
		var regex2 = new RegExp('^([+-]?\\d+)(\\%)?', 'gi');
		while ((value = regex1.exec(match[2])) !== null) {
			if (match[1].toLowerCase() === 'replace') {
				data.replace   = value[1].slice(1, -1);
			} else {
				data.extension = value[1].slice(1, -1);
			}
		};
		while ((value = regex2.exec(match[3])) !== null) { data.index = Number(value[1]) };
	};
	...
	Game_Actor.prototype.characterName = function() {
		...
		var rep  = this.stateReplace();
		var ext  = this.stateExtension();
		var ign  = (this.ignoreStateGraphic() || !VictorEngine.Parameters.ChangeCharset);
		return this.changeStateGraphic(name, rep, ext, ign);
	};
	...
	Game_Actor.prototype.faceName = function() {
		...
		var rep  = this.stateReplace();
		var ext  = this.stateExtension();
		var ign  = (this.ignoreStateGraphic() || !VictorEngine.Parameters.ChangeFaceset);
		return this.changeStateGraphic(name, rep, ext, ign);
	};
	
	VictorEngine.StateGraphics.characterIndex = Game_Actor.prototype.characterIndex
	Game_Actor.prototype.characterIndex = function() {
		if (this.changeStateIndex() && VictorEngine.Parameters.ChangeCharset) {
	...
	Game_Actor.prototype.faceIndex = function() {
		if (this.changeStateIndex() && VictorEngine.Parameters.ChangeFaceset) {
			return ; this.stateGraphicIndex();
		} else {
